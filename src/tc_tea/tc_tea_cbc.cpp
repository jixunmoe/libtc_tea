...
bool CBC_Encrypt(uint8_t *cipher, size_t *p_cipher_len, const uint8_t *plain, size_t plain_len, const uint8_t *key)
{
    ...
    *p_cipher_len = cipher_len;
    ...
    const auto *p_plain = plain + kTeaBlockSize;
    const auto *p_plain_end = plain + plain_len;
    ...
    std::fill_n(&cipher[initial_plain_process_len], kTeaBlockSize * 2 - initial_plain_process_len, 0);
    ...
    if (cipher_len > kTeaBlockSize * 2)
    {
        ...
        {
            tea_encrypt_next_block(&p_plain[-kTeaBlockSize]);
            p_plain += kTeaBlockSize;
        }

        std::array<uint8_t, kTeaBlockSize> last_block{};
        last_block[0] = plain[plain_len - 1];
